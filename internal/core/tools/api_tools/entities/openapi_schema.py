...
class OpenAPISchema(BaseModel):
    ...
    paths: dict[str, Any] = Field(default_factory=dict, validate_default=True, description="工具提供者的路径参数字典")
    ...
    def validate_paths(cls, paths: dict[str, Any]) -> dict[str, Any]:
        ...
        for interface in interfaces:
            ...
            for parameter in interface["operation"].get("parameters", []):
                ...
                if not isinstance(parameter.get("required"), bool):
                    raise ValidateErrorException("parameter.required参数必须为布尔且不为空")
                if not isinstance(parameter.get("in"), str) or parameter.get(
                        "in") not in ParameterIn.__members__.values():
                    raise ValidateErrorException(
                        f"parameter.in参数必须为{'/'.join([item.value for item in ParameterIn])}"
                    )
                if not isinstance(parameter.get("type"), str) or parameter.get(
                        "type") not in ParameterType.__members__.values():
                    raise ValidateErrorException(
