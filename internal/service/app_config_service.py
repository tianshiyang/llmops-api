...
class AppConfigService(BaseService):
    ...
    def get_app_config(self, app: App) -> dict[str, Any]:
        ...
        # 2.TODO校验model_config配置信息，等待多LLM实现的时候，再来完成
        ...
        # 5.校验知识库列表，如果引用了不存在、被删除的知识库，则需要提出数据并更新，同时获取知识库的额外信息
    ...
    def _process_and_validate_datasets(self, origin_datasets: list[str]) -> tuple[list[dict], list[str]]:
